...
public enum BlockType {Flat, Hole, Boner};
...
public class LevelGenerator : MonoBehaviour {
	...
	public int boner_height = 3;

//	private float p_flat  = 0.7f;
//	private float p_hole  = 0.2f;
//	private float p_boner = 0.1f;

	public double p_flat  = 0.2f;
	public double p_hole  = 0.4f;
	public double p_boner = 0.4f;
...
//		init ();
	...
	public uint randomize_params(double speed, double force, double gravity) {
		...
		// var maxDist = (2 * speed * force)/gravity;
		...
		// block_length = (int) rand.Next (2, 20);
		...
		// Debug.Log("desdo - jump "+(maxDist) + " - width "+block_length);

		// boner_height = (int) rand.Next (1, 10);
		boner_height = (int) rand.Next (1, 
								(int)Math.Max(
		...
		// Debug.Log("len "+block_length+"  h "+boner_height+" wat "+(int)Math.Max(
		// 							Math.Min(maxHeight, 10.0),
		// 							1.0
		// 						));
		...
		p_boner = Math.Round(1 - p_flat - p_hole, 1);

		Debug.Log ("flat: "+p_flat + "\t " +"hole: "+ p_hole + "\t " +"obstacle: "+ p_boner);
		return tmp;
	}

	public void set_params(double p_flat, double p_hole, double p_boner, int block_lenght, int boner_height, uint seed) {
		this.p_flat = p_flat;
		this.p_hole = p_hole;
		this.p_boner = p_boner;
		this.block_length = block_lenght;
		this.boner_height = boner_height;
		this.seed = seed;
	...
	void calculate_randon(){
		...
		for (int i = offset; i < randon.Length; i++) {
			randon [i] = BlockType.Boner;
		}
//		randon = randon.OrderBy (x => rand.Next ()).ToArray ();
	}
...
//			if (chunk_offset == 0 && i * block_length < 20) {
			...
			case BlockType.Hole:
				...
				break;
			case BlockType.Boner:
				generate_boner (block_pos);
				break;
			}
		}
...
//		return (int) (seed = (seed * 279470273u) % 0xfffffffb) % max;
	}
	...
	void generate_boner(float block_pos) {
		generate_flat (block_pos);

		float boner_pos = block_pos + block_length / 2;
		for(int i = 1; i <= /*random.Next(0, boner_height)+1*/boner_height; i++){
			GameObject new_sprite = (GameObject)Instantiate (sprite_prefab);
			new_sprite.tag = "Sprite";

			float y_pos = start_y + i * sprite_height;
			new_sprite.transform.position = new Vector2(boner_pos, y_pos);
		}
	}
