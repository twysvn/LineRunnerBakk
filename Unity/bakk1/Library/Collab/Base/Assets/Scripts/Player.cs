...
public class Player : MonoBehaviour {
	...
	private float time_per_action = 1.0f/7.7f;
	...
	void FixedUpdate() {
		...
		if (should_jump) {
			...
			if (levelgen.all_flat || show_rays) {
				...
				// Debug.DrawRay (new Vector3 (transform.position.x, 1, 0), new Vector3 (0, (float)maxHeight, 0), Color.white, 2f);
...
//						Debug.Log ("next_jump_pos " + next_jump_pos);
